...
function getAllUsuarios($conectada) {
    $query = "SELECT id_usu,usuario, nombre, apellidos, fecha_nacimiento FROM usuario"; 
...
function getUsuById($nombre_usuario, $conectada) {
    $query = "SELECT id_usu, nombre, apellidos, fecha_nacimiento FROM usuario WHERE id_usu = ?";
...
function createUsu($datos, $conectada) {
    ...
    $query_check = "SELECT COUNT(*) as count FROM usuario WHERE id_usu = ?";
    $stmt_check = $conectada->prepare($query_check);
    $stmt_check->bind_param("s", $datos['id_usu']);
    $stmt_check->execute();
    ...
    $query = "INSERT INTO usuario (id_usu, contra, nombre, apellidos, fecha_nacimiento) VALUES (?, ?, ?, ?, ?)";
    $stmt = $conectada->prepare($query);
    ...
    if ($stmt->execute()) {
        ...
        return [
            'success' => true,
            'usuario' => getUsuById($datos['id_usu'], $conectada)
        ];
    } else {
        $stmt->close();
        return [
...
function updateUsu($id_usu, $datos, $conectada) {
    ...
    $usuario_actual = getUsuById($id_usu, $conectada);
    if (!$usuario_actual) {
        return [
            'success' => false,
    ...
    if ($stmt->execute()) {
        ...
        return [
            'success' => true,
            'usuario' => getUsuById($id_usu, $conectada)
        ];
    } else {
        $stmt->close();
        return [
...
function deleteUsuByUsername($id_usu, $conectada) {
    // Verificar que el usuario existe
    $usuario_actual = getUsuById($id_usu, $conectada);
    if (!$usuario_actual) {
        return [
    ...
    if ($stmt->execute()) {
        ...
        return [
            'success' => true,
            'mensaje' => "Usuario '$id_usu' eliminado correctamente"
        ];
    } else {
        $stmt->close();
        return [
...
switch($method) { 
    case 'GET': 
        if(isset($_GET['nombre_usuario'])) { 
            ...
            $usu = getUsuById($nombre_usuario, $conectada); 
            echo json_encode($usu ?: ['error' => "No se encontró ningún usuario con nombre '$usuario'"]); 
        } else { 
